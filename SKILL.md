...
| 有一份成型简历，想改好 | **路径 B：审改** |
| 有简历，但要投某个具体岗位 | 路径 B + `references/matching.md`（定向调整） |
| 用户说「我没什么可写的」「没有实习」「经历太平凡」 | 路径 A + `references/sourcing.md` |

两条路都要走**真实性审计**（见下文），这是最容易跳过、但价值最高的环节。

**最容易误判的一种情况**：用户说"我没什么可写的"，于是你以为要做一份缩水版简历。**这是错的判断** —— 见 `references/sourcing.md`，素材几乎总能挖出来，挖不出来也能在 3-7 天内补一个出来。
...
「认真负责」「学习能力强」「沟通能力好」——这些词几乎每份简历上都有。招聘方不会因为你写了就相信你。

**检验方法**：把这句话拿去问"换一个人的简历上是不是也能原样写？" 能 → 就是空话。

替换方法：**用产生这个判断的具体动作，替换掉形容词本身**。

| 空话 | 改成 |
|---|---|
| 学习能力强 | 为准备运营岗位，整理 20 条同类 JD，归纳 Excel、文案、数据整理等高频要求，针对缺口补充学习 |
| 沟通能力好 | 在校园活动中负责参与人员通知、场地对接与现场协调，协助团队按计划完成活动 |
| 认真负责 | 负责 300 条报名信息的核对，发现并修正 17 条重复与缺失记录 |

**同一原则的另一个变体：不要给自己打分。** "Excel 掌握程度 90%"换成"能使用 Excel 完成筛选、排序、基础数据清洗"。百分比没有统一标准，招聘方无法判断；**能做什么是可验证的**。

详见 `references/writing-rules.md` 的「少写形容词，多写动作」和「不要给自己打分」。
...
**如果用户说「我没什么可写的」「没有实习」**：不要接受这个前提，也不要立刻开始写一份缩水简历。先读 `references/sourcing.md` —— 里面列了六类常被忽略的素材来源（课程项目、论文、社团、兼职、竞赛、个人练习），以及「主动补项目」的具体做法：针对目标岗位花 3-7 天做一个有产出物的练习（拆解 10 个同类账号、用公开数据集做一份分析报告、给一个功能写 PRD…）。

**应届生没有实习很正常，完全没有准备才容易被跳过。** 用户手上没有证据时，正确的动作是帮他造一个，而不是把简历删短。
...
如果用户答"都能投""先看看合不合适"——这本身就是问题。**一份什么都写一点的简历，招聘方看不出方向**，最后哪个岗位都不够匹配。

先读 `references/matching.md`，走三步：

1. 找 **10 条同类岗位的 JD**，圈出反复出现的高频词
2. 回用户的经历里**找证据**
3. 按岗位**调整每条经历的侧重点**（同一件组织活动的事，投运营和投行政写法完全不同）

**没有目标岗位的"通用简历"在正式投递时大多要重做**——除非用户明确要海投，那就按 `matching.md` 的多版本策略处理（去掉方向标签和页脚，用专业名或届别做头部）。
...
**应届生简历的四个高频病**（细节和改写示例见 `references/writing-rules.md`）：

- **形容词当内容**：满篇"认真负责""学习能力强""沟通能力好"。检验方法：这句话换到别人简历上是不是也成立？成立就是空话。**换成具体动作**——"学习能力强"改写成"整理 20 条同类岗位 JD，归纳高频要求，针对缺口补充学习"。
- **流水账**：写了"参与 XX 活动""协助整理数据"，但读者看完还是不知道你具体做了什么。**每条至少要回答三个问题**：做了什么 / 怎么完成的 / 产出了什么。
- **自我打分**："Excel 掌握程度 90%""Python 熟练度 ★★★★☆"。百分比没有统一标准，招聘方无法判断，只能当噪音。**改成能做什么**——"能使用 Excel 完成筛选、排序、基础数据清洗"。
- **零岗位关键词**：整份简历找不到目标 JD 里的高频词（Excel、数据整理、活动执行…）。招聘方是**快速扫描**不是逐字细读，关键词缺失会让他三秒内判定"不匹配"。对着 JD 圈词，回经历里找证据。
...
交付前把这张单子过一遍。**这些细节不一定加分，但出错一定影响第一印象。**

**内容层**（区别于下文「验证（必做）」里的技术层检查）：

- [ ] **求职方向清楚** —— 头部或求职意向能让招聘方一眼看出投什么岗位
- [ ] **有岗位关键词** —— 对着目标 JD 圈出的高频词，简历里出现了几个
- [ ] **每条经历都有具体动作** —— 不是写到"参与/协助"就结束
- [ ] **有真实产出或结果** —— 有数据写数据，没数据写真实产出，不编增长率
- [ ] **没有自我评价式空话** —— "认真负责""抗压能力强"这类清理掉
- [ ] **没有自我打分** —— "Excel 90%"换成"能完成 XX 任务"
- [ ] **联系方式正确** —— 电话、邮箱逐个核对；邮箱别用过于随意的昵称
- [ ] **文件名规范** —— `姓名-应聘岗位-电话.docx`，不要「我的简历最终版」「最新版 2」「新建文档」

**技术层**：见下文「验证（必做）」的 6 项，跑 `scripts/check_residue.py` 能覆盖大部分。
...
6. **页眉页脚方向一致性** —— 定向版简历的头部和页脚**通常各印一份目标方向**（头部是「产品运营 / 数据分析（2027届）」，页脚是「张三 | 产品运营 / 数据分析 | 第 1 页」）。

   改方向时只改头部、漏改页脚，就会得到一份**自相矛盾**的文件：头部写产品运营、页脚写竞品研究。这个故障在正文里查不出来 —— 页脚是独立的 XML 部件（`word/footer1.xml`），正文文本提取根本不覆盖它；而且两个方向标签各自看着都「合理」，不放在一起对比就发现不了。**投出去就是硬伤**，HR 翻一眼页脚就看见了。

   **改方向时必须同时改 `word/document.xml` 和 `word/footer*.xml`。** 跑 `scripts/check_residue.py` 会自动比对并报错（`check_footer_direction`）。
...
- `references/matching.md` —— 岗位匹配：从 JD 提关键词、不同岗位关注什么、多版本策略
- `references/sourcing.md` —— 没有实习 / 经历普通怎么办：素材清单与主动补项目
